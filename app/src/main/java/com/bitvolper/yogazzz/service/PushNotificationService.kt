package com.bitvolper.yogazzz.service
...
class PushNotificationService: FirebaseMessagingService() {
    ...
    companion object {
        enum class CHANNEL_ID {
            DAILY_REMINDER, FEEDBACK_APP_UPDATES
        }
    }
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        coroutineScope.launch {
            repository.readDailyReminder.collect {
                if (it.isEnabled) {
                    showNotification(channelId = CHANNEL_ID.DAILY_REMINDER.name, title = title, text = text, notificationId = 1)
                }
            }

            repository.readFeedbackAppUpdates.collect {
                if (it.isEnabled) {
                    showNotification(channelId = CHANNEL_ID.FEEDBACK_APP_UPDATES.name, title = title, text = text, notificationId = 2)
                }
            }
        }
