...
fun RecommendationScreen(modifier: Modifier = Modifier,
    ...
    Column(modifier = modifier
        ...
        when (recommendationUIState) {
            ...
            is Resource.Failure -> {
                Text(text = recommendationUIState.throwable.toString())
            }
            is Resource.Success -> {
                Column(modifier = modifier
                    .fillMaxSize()
                    ...
                    LazyColumn {
                        items(recommendationUIState.data.data?.size ?: return@LazyColumn) {
                            RecommendationCard(
                                recommendation = recommendationUIState.data.data[it] ?: return@items
                            )
                        }
                    }
                }
